...
import { paymentsOptions, sourceOptions, statusOptions } from './add-invoice.model';
import { InvoicingService } from '../invoicing.service';

@Component({
  selector: 'app-add-invoice',
  templateUrl: './add-invoice.component.html',
  styleUrls: ['./add-invoice.component.scss', '../../../assets/stylesheets/detail-page.scss'],
})
export class AddInvoicingComponent implements OnInit {
  ...
  ngOnInit() {
    this.invoicingService.showInvoiceFormError = false;
    this.invoicingService.invoiceDataForm = null;
    this.getTagsList();
    this.getModalData();
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.saleOrderId !== 0) {
        ...
        this.invoicingService.getPlansModel();
  ...
  changeStatusSub(item) {
    ...
    if(this.saleOrderId && this.saleOrderId !== 0) {
      ...
      this.subscriptionService.updateSingleFieldInvoices(payload,this.saleOrderId).subscribe((response) => {
        this.invoicingService.invoiceDataForm.patchValue({
          status: item.value
        });
        this.statusOptions.forEach((element, index) => {
          if (this.invoicingService.invoiceDataForm.value.status === element.value) {
  ...
  getModalData(): void {
    ...
    this.subscriptionService.getModalData().subscribe((response) => {
      ...
      this.salesOrderList = response[6].body.results.map((obj) => {
        return ({ label: obj.sale_order_id, value: obj.id })
      });
  ...
  getSaleOrderData() {
    this.invoicingService.singleInvoicing(this.saleOrderId).subscribe((response) => {
      this.allData = response.data;
      this.invoicingService.getPlansModel(response.data);
      this.checklistData = [];
      this.allData.sales_team && this.onSalesTeamChange({value:this.allData.sales_team.id}, false);
      this.statusOptions.forEach((element, index) => {
        if (this.invoicingService.invoiceDataForm.value.status === element.value) {
          this.activeIndex = index;
        }
      ...
      if(response.data.lines) {
        response.data.lines.forEach(element => {
          this.checklistData.push({
            selected: false, ...element ,
            invoice: this.saleOrderId
          });
        });
      }
    }, err => this.showError(err));
 ...
 */
  ...
  deleteChecklistItem(in_id = null) {
    const payload = {invoice_line_ids: []}
    if(!in_id) {
      this.checklistData.forEach(obj => {
        if (obj.selected) {
          payload.invoice_line_ids.push(obj.id);
        }
      });
    }
    else {
      payload.invoice_line_ids.push(in_id.id);
    }
    this.invoicingService.bulkUpdateInvoiceLine(payload).subscribe((response) => {
      this.messageService.add({ severity: 'success', summary: 'Success', detail: "Invoice lines deleted successfully" });
  ...
  addChecklistItem() {
    this.saleData = {
      selected: false, id:0, invoice: this.saleOrderId, product: null, description: null, quantity: null, unit_price: null, product_tax: null
    };
  ...
  onSalesTeamChange(event, reset=true) {
    ...
    if(reset) {
      this.invoicingService.invoiceDataForm.patchValue({sales_person: null})
    }
  }
  ...
  onEditTransactionData(rowData) {
    this.saleData = {
      ...
      product_tax: (rowData.product_tax && rowData.product_tax.id) ? rowData.product_tax.id : rowData.product_tax
    };
    this.addOrderLine = true;
  }
  ...
  addOrderLineRow(payload) {
    delete payload.id;
    this.invoicingService.createInvoicingLine(payload).subscribe((response) => {
      this.addOrderLine = false;
      this.total = this.checklistData.length;
      this.appService.updateHeaderName({ name: 'Invoicing', count: this.total });
      this.messageService.add({ severity: 'success', summary: 'Success', detail: "Invoice line created successfully" });
      this.getSaleOrderData();
    }, err => this.showError(err));
  }
  ...
  */
  ...
  addToTable() {
    if (this.saleOrderId === 0) {
      if (this.invoicingService.invoiceDataForm.valid) {
        this.addFormInfo(this.invoicingService.preparePlanPayload(), false);
      }
      else {
      ...
      if (this.saleData.id === 0) {
        ...
        const payload = { ...this.saleData };
        this.invoicingService.updateInvoicingLine(payload, payload.id).subscribe((response) => {
          this.addOrderLine = false;
          this.messageService.add({ severity: 'success', summary: 'Success', detail: "Invoice line updated successfully" });
          this.getSaleOrderData();
        }, err => this.showError(err));
      }
    }
  ...
  updateFormInfo(payload) {
    this.invoicingService.updateInvoicing(payload, this.saleOrderId).subscribe((response) => {
      this.showResMessage(response);
      this.router.navigate(["subscription/invoicing"]);
    }, err => this.showError(err));
  }

  addFormInfo(payload, close = true) {
    this.invoicingService.createInvoicing(payload).subscribe((response) => {
      if (close) {
        this.showResMessage(response);
        this.router.navigate(["subscription/invoicing"]);
      }
  ...
  addUpdateChecklist() {
    this.invoicingService.showInvoiceFormError = true;
    if (this.invoicingService.invoiceDataForm.valid) {
      const payload = this.invoicingService.preparePlanPayload();
      if (this.saleOrderId !== 0) {
  ...
  showError(serverError) {
    this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Selected status is not valid choice'});
  }
